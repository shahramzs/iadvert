...
const [notificationForm, setNotificationForm] = React.useState(false);
const [advert, setAdvert] = React.useState([]);

React.useEffect(() => {
  const SERVER_URL = "http://127.0.0.1:8000";
  const url = `${SERVER_URL}/src/api/admin/notification.php`;
...
const handleFormSubmit =  async (e) => {
  ...
  await axios({
    method: 'POST',
    url : `${SERVER_URL}/src/api/admin/insertNotification.php`,
    data : formData,
    responseType: 'json',
...
.then(function(response){
    ...
    if(response.data !== 'not ok'){
      ...
      let title = response.data[0];
      let user = response.data[1];
      let message = response.data[2];
      let time = moment();

      const newList = advert.concat({ title , user , message, time});
      setAdvert(newList);
...
const deleteNotification = async (id) => {
 ...
 setAdvert(newId);
  ...
  await axios({
    method: 'POST',
    url : `${SERVER_URL}/src/api/admin/deleteNotification.php`,
    data : formData,
    responseType: 'json',
    config : {headers : { 'Content-Type': 'multipart/form-data' }},
})
...
React.useEffect(() => {
  ...
  return (
    <div className={classes.root}>
       ...
       edge="start"
        ...
        <IconButton
          ...
          <IconButton
            ...
            <IconButton aria-label="show 17 new notifications" color="inherit" onClick={()=>setNotificationForm(true)}>
                <NotificationsIcon />
            </IconButton>
            <IconButton
              edge="end"
              ...
              <Grid item xs={12} key={i}>
                <Card className={classes.root} variant="outlined">
                  <CardContent>
                    ...
                    <CardActions>
                    <Button variant="outlined" color="secondary" size="small" onClick={()=>deleteNotification(info.id)}>delete</Button>
                  </CardActions>
                  </CardContent>
                </Card>
              </Grid>
      ...
      <Rodal visible={notificationForm} onClose={()=> setNotificationForm(false)} animation='door' width={800} height={450}>
              <Box>
                <h5 style={{textAlign:'center'}}>Send Notification</h5>
                <form className={classes.form} onSubmit={(e) => {handleFormSubmit(e)}}>
                    ...
                    <div style={{display:'flex',flexDirection:"row",margin:'2rem auto'}}>
                        <Button size="large" color="primary" style={{marginRight:'5rem'}} type="submit" >Add Notification</Button>
                        <Button size="large" color="secondary" onClick={()=>setNotificationForm(false)} style={{marginLeft:'5rem'}}>Cancel</Button>
                    </div>
                </form>  
              </Box>                  
            </Rodal>
