...
React.useEffect(() => {
  const SERVER_URL = "http://127.0.0.1:8000";
  const url = `${SERVER_URL}/src/api/allDiscount.php`;
...
const handleExpireChange = (e) => {
    ...
    if(e.target.value !== ''){
     setError({expire:false})
    }
    if(moment(start).isAfter(e.target.value) ){
        setError({expire:true, expire2:true});
        return false;
    }else{
        setError({expire:false, expire2:false});
    }
};
...
const handleFormSubmit = async (e) => {
  ...
  if(moment(start).isAfter(expire) ){
    setError({expire:true, expire2:true});
    return false;
}else{
    setError({expire:false, expire2:false});
  ...
  setProgress(true);
  const SERVER_URL = "http://127.0.0.1:8000";
  const discountCode = window.btoa(Array.from(window.crypto.getRandomValues(new Uint8Array(15 * 2))).map((b) => String.fromCharCode(b)).join("")).replace(/[+/]/g, "").substring(0, 15);
  ...
  await axios({
       method: 'POST',
       url : `${SERVER_URL}/src/api/insertDiscount.php`,
       data : formData,
       responseType: 'json',
       config : {headers : { 'Content-Type': 'multipart/form-data' }},
   ...
   .then(function(response){
       ...
       if(response.data !== 'not ok'){
         ...
         let title = response.data[1];
         let text = response.data[3];
         let start = response.data[4];
         let expire = response.data[5];
         let price = response.data[2];
         let discountCode = response.data[6];
         let time = moment();

         let user = sessionStorage.getItem('user');
         const newList = advert.concat({ user , url , token, title, text, start, expire, price, discountCode, time});
         setAdvert(newList);
...
// console.log('may deleted',newId);
  ...
  let formData = new FormData();
   ...
   await axios({
    method: 'POST',
    url : `${SERVER_URL}/src/api/deleteDiscount.php`,
    data : formData,
    responseType: 'json',
    config : {headers : { 'Content-Type': 'multipart/form-data' }},
})
...
React.useEffect(() => {
  document.title  = `${sessionStorage.getItem('user')} / Add or Remove Discount`;
  return () => {
    ...
    <div className={classes.root}>
      ...
      >
        ...
        <List>
            ...
            <ListItem button onClick={discount} style={{background:'silver'}} >
                <ListItemIcon>
                     <LocalOfferTwoToneIcon />
                </ListItemIcon>
      ...
      <main className={classes.content}>
        ...
        {advert && advert === 'noDis' || advert && advert.length === 0 ? <Alert severity="error" style={{width:'50%',margin:'0 auto'}}>
                                                <AlertTitle>No Discount Found!</AlertTitle>
                                                 Discount Don't found! — <strong>check it out!</strong>
                                         </Alert> : 
           <React.Fragment>
     {advert && advert.map((info, i) =>
          <React.Fragment >
              <Grid item xs={4} key={i} >
                <Card className={classes.root1} >
                    ...
                    />
                    <CardContent>
                    <Typography variant="body2" color="textSecondary" component="p" >Titel : {info.title}</Typography>
                    <Typography variant="body2" color="textSecondary" component="p" >Token : {info.token}</Typography>
                    <Typography variant="body2" color="textSecondary" component="p" >Discount : <strong style={{color:'red'}}>{info.price}%</strong></Typography>
                    <Typography variant="body2" color="textSecondary" component="p" >Discount Code : <strong style={{color:'blue'}}>{info.discountCode}</strong></Typography>
                    <Typography variant="body2" color="textSecondary" component="p" >Start Time : <strong style={{color:'#ff8080'}}>{moment(info.start).format('LLLL')}</strong></Typography>
                    <Typography variant="body2" color="textSecondary" component="p" >Discount : <strong style={{color:'#ff8080'}}>{moment(info.expire).format('LLLL')}</strong></Typography>
                    </CardContent>
      ...
      <Rodal visible={visible} onClose={()=> setVisible(false)} animation='rotate' width={500} height={500}>
            ...
            <Rodal visible={visible2} onClose={()=> setVisible2(false)} animation='door' width={800} height={500} customStyles={{overflow: 'auto'}} >
              <Box>
                <h5 style={{textAlign:'center'}}>Add Discount To your Business Advert</h5>
                <form className={classes.form} onSubmit={(e) => {handleFormSubmit(e)}}>
                    ...
                    />
                    <Typography variant="caption" display="block" gutterBottom  className={classes.textToken}>
                       You can add some information about your discount.
                    </Typography>
                    ...
                    />
                    <TextField
                        error = {error.expire ? true : false}
                        onChange={handleExpireChange}
                        helperText={error.expire2 ? "expire time can not come before start time!" : ''}
                        id="datetime-local"
                        label="Expire time"
                        type="datetime-local"
                        className={classes.textField}
                    ...
                    <input
                      ...
                      type="file"
                      name="Image Of Discount"
                      onChange={handleImageChange}
                    />
                    <label htmlFor="contained-button-file">
                      <Button variant="contained" color="primary" component="span">
                        Upload Image {' '}<LocalOfferTwoToneIcon />
                      </Button> 
                    ...
                    <div style={{display:'flex',flexDirection:"row",margin:'2rem auto'}}>
                        <Button size="large" color="primary" style={{marginRight:'5rem'}} type="submit" >Add Discount</Button>
                ...
                >
                  ...
                  <DialogContent>
                    <DialogContentText id="alert-dialog-description">
                      You add discount successfully. <strong>you can add more discount here.</strong>
                    </DialogContentText>
                  </DialogContent>
                  <DialogActions>
            ...
            >
              <DialogTitle id="alert-dialog-title">{"Add Discount Failed"}</DialogTitle>
              <DialogContent>
                <DialogContentText id="alert-dialog-description">
                  You add discount unsuccessfully. <strong>you can add more discount here.</strong>
                </DialogContentText>
              </DialogContent>
              <DialogActions>
