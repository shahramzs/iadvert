...
React.useEffect(() => {
  const SERVER_URL = "http://127.0.0.1:8000";
  const url = `${SERVER_URL}/src/api/location-user.php`;
  const fetchData = async () => {
  const {data} = await axios.get(url+'?user='+sessionStorage.getItem('user'));
  setAdvert(data);

  if(data.locationUser && data.locationUser.length === 0){
    setDialog(true)
  }else{
    setDialog(false)
  }
...
React.useEffect(() => {
  document.title  = `${sessionStorage.getItem('user')} / Advert Location`;
  return () => {
    ...
    <div className={classes.root}>
      ...
      >
        ...
        <List>
            ...
            <ListItem button onClick={location} style={{background:'silver'}}>
                <ListItemIcon>
                     <LocationOnIcon />
                </ListItemIcon>
        ...
        <Box component="span" m={1}> 
          <Map className="map-fragment-user" center={[40,-170]} zoom={2}>
              <TileLayer
                  ...
                  />
{advert.locationUser && advert.locationUser.map((info, i) => 
                  <Marker position={[info.lat, info.lng]} key={i}>
                      <Popup style={{width:"100%",height:"100vh"}}>
                        <div>
                        <p><strong>{info.city}{''}</strong>/ {info.streetAddress}</p>
      ...
      <Dialog
        ...
        >
          <DialogTitle id="alert-dialog-title">{"User Submit Location"}</DialogTitle>
          <DialogContent>
            <DialogContentText id="alert-dialog-description">
              You do not submit any location for your business yet.
            </DialogContentText>
          </DialogContent>
      </Dialog>
