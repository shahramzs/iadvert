...
React.useEffect(() => {
  const SERVER_URL = "http://127.0.0.1:8000";
  const url = `${SERVER_URL}/src/api/allVideos.php`;
...
// console.log('may deleted',newId);
  ...
  let formData = new FormData();
   ...
   await axios({
    method: 'POST',
    url : `${SERVER_URL}/src/api/deleteVideo.php`,
    data : formData,
    responseType: 'json',
    config : {headers : { 'Content-Type': 'multipart/form-data' }},
})
...
.catch(function(response){
  ...
  const[moreVideo, setMoreVideo] = React.useState('')
  ...
  const handleVideoChange = (e) => {
   ...
   const handleFormSubmit = async (e) => {
    ...
    if(moreVideo === ''){
      setError({moreVideo:true})
      return false;
    }else{
      setError({moreVideo:false})
    }
    setProgress(true);
    ...
    let formData = new FormData();
     ...
     formData.append('video', moreVideo);
  
    await axios({
         method: 'POST',
         url : `${SERVER_URL}/src/api/insertMoreVideo.php`,
         data : formData,
         responseType: 'json',
         config : {headers : { 'Content-Type': 'multipart/form-data' }},
...
React.useEffect(() => {
  document.title  = `${sessionStorage.getItem('user')} / Videos`;
  return () => {
    ...
    <div className={classes.root}>
      ...
      >
        ...
        <List>
            ...
            <ListItem button onClick={video} style={{background:'silver'}}>
                <ListItemIcon>
                     <VideoLibraryIcon />
                </ListItemIcon>
      ...
      <main className={classes.content}>
        ...
        {advert && advert.length === 0 ? <Alert severity="error" style={{width:'50%',margin:'0 auto'}}>
                                                        <AlertTitle>No Video Found!</AlertTitle>
                                                        Video Does'nt found! — <strong>check it out!</strong>
                                                      </Alert>  : 
           <React.Fragment>
              ...
              <Grid item xs={3} key={i}>
                <Card className={classes.root1}>
                    ...
                    <video width="320" height="240" controls style={{width:'100%'}}>
                        <source src={info.url.replace('../public','')} type="video/mp4"/>
                      </video>
                      <CardContent>
        ...
        <Zoom
          ...
          unmountOnExit
        >
        <Fab color="secondary" aria-label="add video" className={classes.fab} onClick={()=>setVisible2(true)}>
           <VideocamIcon />
        </Fab>
      </Zoom>
      </main>
      ...
      <Rodal visible={visible} onClose={()=> setVisible(false)} animation='rotate' width={500} height={500}>
            ...
            <Rodal visible={visible2} onClose={()=> setVisible2(false)} animation='door' width={800} height={400}>
              <Box>
                <h5 style={{textAlign:'center'}}>Add More Video To your Business Advert</h5>
                <form className={classes.form} onSubmit={(e) => {handleFormSubmit(e)}}>
                    ...
                    <label htmlFor="contained-button-file">
                      <Button variant="contained" color="secondary" component="span">
                        Upload Video {' '}<VideocamIcon />
                      </Button>
                      <Typography>{moreVideo.name} {' '} {moreVideo.size}</Typography>
                      {error.moreVideo ? <p style={{color:'red'}}>Please select an video</p> : ''}
                    </label>
                    <Typography variant="caption" display="block" gutterBottom className={classes.addImage}>
                      You can add more video to your advert here.
                    </Typography>
                    <div style={{display:'flex',flexDirection:"row",margin:'2rem auto'}}>
                        <Button size="large" color="primary" type="submit" style={{marginRight:'5rem'}}>Add Video</Button>
                ...
                >
                  ...
                  <DialogContent>
                    <DialogContentText id="alert-dialog-description">
                      You upload more video successfully. <strong>you can add more video here.</strong>
                    </DialogContentText>
                  </DialogContent>
                  <DialogActions>
            ...
            >
              ...
              <DialogContent>
                <DialogContentText id="alert-dialog-description">
                  You upload more video unsuccessfully. <strong>you can add more video here.</strong>
                </DialogContentText>
              </DialogContent>
              <DialogActions>
                <Button onClick={uploadVideo} color="primary">
                  OK
                </Button>
              </DialogActions>
        </Dialog>
